...
/**
 * The studio week grid: pick free time by tapping it, adjust below, book as
 * many slots as you need. Used by the wizard's step 2 AND the Studio tab, so
 * booking a room feels the same whether or not a shoot is involved.
 */
export function StudioWeekPicker({
  studios,
  blocks,
  slots,
  onSlotsChange,
  ...
  /** Several rooms, several days, or several windows a day are all allowed. */
  slots: StudioSlot[]
  ...
  if (studios.length === 0) {
    return (
      <p className="text-[13px] text-muted-foreground">
        No studios are set up yet. The tech lead can add them in the Tech Console.
      </p>
    )
  }
  ...
  return (
    <div className="min-w-0 space-y-4">
      ...
      {slots.length > 0 ? (
        ...
        <p className="text-[13px] text-muted-foreground">
          {emptyHint ?? 'Nothing picked yet. Tap any free time above to hold the room.'}
        </p>
      )}
    </div>
  )
